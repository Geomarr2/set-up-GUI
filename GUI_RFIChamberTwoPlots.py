...
class GUIOptions(object):
    def __init__(self,root,parent):
       ...
       # self.head.readFromFile('C:/Users/geomarr/Documents/GitHub/set-up-GUI/RFIDummyDataset/RFIscan_100000000_20181015T144156552784.rfi')
        self.headerFile = []
        self.dataFile = []
...
#Load in vereything and seperate header and data file
        ...
        self.G_LNA = self.cal_GainCircuit(Stop_freq, Start_freq)
        self.CCF = self.get_CCF(Stop_freq, Start_freq)
        self.original_data = self.loadDataFromFile(Start_freq,Stop_freq,dataFile)
        self.scaling_factor = 500
        self.calibrateData(self.read_reduce_Data(self.original_data[1], Start_freq, Stop_freq,self.scaling_factor), Stop_freq, Start_freq,self.G_LNA,self.CCF, color_data_set1)
...
#Load in vereything and seperate header and data file
        ...
        self.G_LNA2 = self.cal_GainCircuit(Stop_freq, Start_freq)
        self.CCF2 = self.get_CCF(Stop_freq, Start_freq)
        self.original_data2 = self.loadDataFromFile(Start_freq,Stop_freq,dataFile)
        self.scaling_factor2 = 500
        self.calibrateData(self.read_reduce_Data(self.original_data2[1], Start_freq, Stop_freq,self.scaling_factor2), Stop_freq, Start_freq,self.G_LNA2,self.CCF2, color_data_set2)
    ...
    def calibrateData(self, reduced_data, upperfreq, lowerfreq,G_LNA, CCF, color_data_set):
       tstart = time.time()
...
# set the display sample size depending on the display bandwidth and resolution  
        ...
        spec_max = [np.max(spectrum[(i*x):(x*i+x)]) for i in range (scaling_factor)]
        spec_mean = [np.mean(spectrum[(i*x):(x*i+x)]) for i in range (scaling_factor)]
        spec_min = [np.min(spectrum[(i*x):(x*i+x)]) for i in range (scaling_factor)]
...
# NEED TO PUT THIS IN A SEPERATE FILE
    ...
    def zoomActivate(self):
        ...
        if zoom_Bandwidth >= 35*1e6:
            ...
            self.calibrateData(reduced_data, self.zoom_Stop_freq, self.zoom_Start_freq,self.G_LNA,self.CCF,color_data_set1)
        else: 
    ...
    def zoomActivate2Data(self):
        ...
        if zoom_Bandwidth >= 35*1e6:
            ...
            self.calibrateData(reduced_data, self.zoom_Stop_freq, self.zoom_Start_freq,self.G_LNA2,self.CCF2,color_data_set2)
        else: 
            tkMessageBox.showwarning(title="Warning", message="Maximum zoom bandwidth must be more than acquisition bandwidth which is %d MHz."%(int(round(self.Bandwidth/1e6))))
    ...
    def plot_data(self,reduced_data,yaxis_label,color_data_set, Start_freq, Stop_freq):
        ...
        if self.original == True:
            ...
            if self.zoom_trigger1 == True:
                ...
                #mean_plot, = self.fig_plot.semilogx(reduced_data[0]/1e6,reduced_data[3], color=color_data_set[2])
                ...
                max_plot, = self.fig_plot.semilogx(reduced_data[0]/1e6,reduced_data[1], color=color_data_set[0])
                min_plot, = self.fig_plot.semilogx(reduced_data[0]/1e6,reduced_data[2], color=color_data_set[1])
               # mean_plot, = self.fig_plot.plot(reduced_data[0]/1e6,reduced_data[3], color=color_data_set[2])
                self.fig_plot.set_ylabel(yaxis_label)#("Electrical Field Strength [dBuV/m]")#('Power [dBm]')
                self.fig_plot.set_xlabel("Frequency (MHz)" )#(resolution %.3f kHz)"%1)
                #self.fig_plot.legend([max_plot,min_plot,mean_plot],['max', 'min', 'mean'])
            ...
            self.fig_plot.legend([max_plot,min_plot],[maxleg, minleg])
            self.canvas.draw()       
